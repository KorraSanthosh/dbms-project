...
# Load .env for local development (no-op if not present / in cloud)
load_dotenv()
...
# Connect to MongoDB
# Reads MONGODB_URI from .env (local) or Streamlit secrets (deployed)
@st.cache_resource
def init_connection():
    uri = os.environ.get("MONGODB_URI") or st.secrets.get("MONGODB_URI")
    if not uri:
        st.error("❌ MONGODB_URI not found. Set it in .env (local) or Streamlit secrets (deployed).")
        st.stop()
...
/* ── Drug input cards (avoids ghost empty-box issue) ── */
.drug-card {
    background: var(--card);
    border: 1px solid var(--border);
    border-radius: 16px;
...
.drug-card:hover { border-color: rgba(0,240,255,.35); box-shadow: 0 0 24px rgba(0,240,255,.08); }
.drug-card-title {
...
# ──────────────────────────────────────────────────────────────────────
    ...
    with col1:
        st.markdown('<div class="drug-card"><p class="drug-card-title">💊 First Drug</p>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="drug-card"><p class="drug-card-title">💊 Second Drug</p>', unsafe_allow_html=True)
    ...
    if show_alerts:
        if drug1 and drug2:
            try:
                raw_alerts = list(alerts_col.find({
                    "$or": [
                        {"drug1": drug1, "drug2": drug2},
                        {"drug1": drug2, "drug2": drug1}
                    ]
    ...
    if find_alt:
        if alt_drug:
            with st.spinner("Searching alternatives…"):
                try:
                    result = alternatives_col.find_one({"drug": alt_drug}, {"_id": 0})
